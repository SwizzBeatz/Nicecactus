...
Tournament_List = async (token) => {
    try {
      const tournament_list_response = await get(tournament_list_url, {headers:{'x-access-token': token}})
...
Participant_List = async (url, token) => {
  try {
    const participants_list_response = await get(url, {headers:{'x-access-token': token}})
