...
async function main() {
  ...
  Tournament_List = async token => {
    try {
      const tournament_list_response = await axios.get(tournament_list_url, {
      ...
      for (var tournament of tournamentlist) {
        ...
        const participants_list_url =
          "https://api.nicecact.us/matchmaking/tournament/" +
          tournament_id +
          "/participants";
  ...
  Participant_List = async (url, token) => {
    try {
      const participants_list_response = await axios.get(url, {
        headers: { "x-access-token": token }
      });
      ...
      for (var participant of participants_list) {
        ...
        //console.log(participants_list_response.data.data[0].name);
        participant_list_results.push({
          id: participant_id,
          name: participant_name
        });
      }
      //console.log(response.data.data);
      return participant_list_results;
    } catch (error) {
      console.log(error.message);
    }
  };
  ...
  const results_tournaments_list = await Tournament_List(accessToken);
  //console.log(results_tournaments_list);

  let generalTab = [];
  for (let i = 0; i < results_tournaments_list.length; i++) {
    //console.log(results_tournaments_list[i]);

    let temp = await Participant_List(
      results_tournaments_list[i].url,
      accessToken
    );
    generalTab.push({
      Tournament_Name: results_tournaments_list[i].TournamentName,
      Participant: temp
    });
  }
